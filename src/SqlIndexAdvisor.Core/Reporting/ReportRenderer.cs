...
public static class ReportRenderer
{
	...
	/// The JSON includes schema version, dialect, statement cost, operator count,
	/// and a list of recommendations with rule IDs, table, key columns, include columns,
	/// estimated impact, confidence, rationale, the generated CREATE statement, and DDL.
	/// </summary>
	/// <param name="plan">The execution plan containing dialect and cost information.</param>
	/// <param name="recs">The list of index recommendations to include in the report.</param>
	/// <param name="schemaVersion">The schema version to include in the output.</param>
	/// <returns>A JSON‑formatted report string.</returns>
	/// <exception cref="ArgumentNullException">Thrown when plan, recs, or schemaVersion is null.</exception>
	public static string RenderJson(ExecutionPlan plan, IReadOnlyList<IndexRecommendation> recs, string schemaVersion = "1.0")
	{
		...
		var payload = new
		{
			...
			recommendations = recs.Select((r, index) => new
			{
				id = $"rec-{index + 1}",
				table = r.Table,
				...
				ddl = r.ToCreateStatement(plan.Dialect),
